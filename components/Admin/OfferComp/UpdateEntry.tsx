...
const UpdateEntry = () => {
    ...
    if (roll_no == "") {
      ...
      if (response.data == null) {
        ...
        for (let i = 0; i < response.data.offers.length; i++) {
          ...
          response.data.offers[i]["packages"] =
            response.data.offers[i]["package"];
        }
  ...
  const submit = async (i: number) => {
    ...
    for (let j = 0; j < drive.length; j++) {
      if (
        drive[j]["role"] == stu_info[i]["role"] &&
        drive[j]["package"] == stu_info[i]["packages"]
      ) {
    ...
    if ((upvote = 0)) {
      ...
      const response = await axios.post(
        `${server}/add/admin/student/updateoffer`,
        body,
        {
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="w-full mt-5 border-t-2 pt-3">
      <div className="border-b-2 border-gray-900 py-2">
        <div className="flex flex-row flex-wrap">
          <h3 className="mt-2 mb-2 text-lg sm:text-xl font-semibold text-gray-900 mr-1">
            Get Offers Details Of Student :{" "}
          </h3>
          <label className="relative cursor-pointer mb-2 mt-2">
            ...
            />
            <span className="text-sm text-opacity-80 text-black bg-white absolute left-5 top-1 px-1 transition duration-200 input-text">
              Roll No*
            </span>
          </label>
        </div>
        <div className="w-full flex flex-row flex-wrap justify-between">
          {loading ? (
            <button className="px-3 py-1 rounded bg-gray-400 sm:text-xl text-white mb-3 flex flex-row items-center">
              Get Data
            ...
            <button
              onClick={get_info}
              className="px-3 py-1 rounded bg-green-600 sm:text-xl text-white hover:bg-green-700 mb-3"
            >
              Get Data
            </button>
          )}
          <button
            onClick={clear_info}
            className="px-3 py-1 rounded bg-red-600 sm:text-xl text-white hover:bg-red-700 mb-3"
          >
            Clear
          </button>
        </div>
            ...
            <table className="table-fixed border-separate border-spacing-x-4 border-spacing-y-2 mt-3">
              <thead>
                <tr className="text-lg font-semibold text-gray-900">
                  <th className="border-b-2 border-slate-600 pr-1 sm:pr-2">
                    Company
                  </th>
                  <th className="border-b-2 border-slate-600 pl-1 sm:pl-2">
                    Package
                  </th>
                  <th className="border-b-2 border-slate-600 pl-1 sm:pl-2">
                    Role
                    ...
                    >
                        ...
                        <td className="pr-1 sm:pr-2">
                          <label className="relative cursor-pointer mb-4">
                            <input
                              ...
                              className="h-8 px-2 text-sm text-black bg-white border-gray-500 border-2 rounded-md border-opacity-50 outline-none focus:border-red-600 placeholder-gray-700 placeholder-opacity-0 transition-duration-200"
                            />
                            <span className="text-sm text-opacity-80 text-black bg-white absolute left-5 top-0 px-1 transition duration-200 input-text">
                              Company Name
                            </span>
                          </label>
                        </td>
                      ...
                      {update ? (
                        <td className="pr-1 sm:pr-2">
                          <label className="relative cursor-pointer mb-4">
                            ...
                            />
                            <span className="text-sm text-opacity-80 text-black bg-white absolute left-0 top-0 px-1 transition duration-200 input-text">
                              Package
                            </span>
                          </label>
                        </td>
                      ...
                      {offer_letter ? (
                        <td className="pl-1 sm:pl-2">
                          <button
                            className="flex flex-row items-center my-2 px-3 py-1 rounded bg-green-600 text-white hover:bg-green-700"
                            onClick={() => {
                        ...
                        <td className="pl-1 sm:pl-2 my-2">
                          <h3 className="my-2">Not Uploaded</h3>
                        </td>
                      )}
                      {update ? (
                        <td className="pl-1 sm:pl-2 flex flex-row ">
                          <button
                            onClick={() => submit(i)}
                            className="items-center my-2 px-3 py-1 rounded bg-blue-600 text-white hover:bg-blue-700"
                          >
                            Update
                          </button>
                          <button
                            ...
                            className=" my-2 ml-1 rounded-full px-1 bg-red-600 text-white hover:bg-red-700"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                        ...
                        <td className="pl-1 sm:pl-2">
                          <button
                            ...
                            className="flex flex-row items-center my-2 px-3 py-1 rounded bg-blue-600 text-white hover:bg-blue-700"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
                          ...
                          <button
                            ...
                            className="flex flex-row items-center my-2 px-3 py-1 rounded bg-red-600 text-white hover:bg-red-700"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
