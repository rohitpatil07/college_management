...
const StuAssignment = ({ subject_id, subject_name }: any) => {
    ...
    for (let i = 0; i < response1.data.length; i++) {
      ...
      if (response.data && response.data.length != 0) {
        for (let j = 0; j < response.data.length; j++) {
          if (response.data[j]["assignment_id"] ==response1.data[i]["assignment_id"]) {
            response1.data[i]["submitted"] = true;
            response1.data[i]["submitted_file_name"] = response.data[j]["file_name"];
            response1.data[i]["submitted_file_type"] = response.data[j]["file_type"]
          }
        }
      }
    }
              ...
              <tbody>
                {assign.map(
                  (
                    ...
                    <tr>
						 <td className="border-l-2 border-l-slate-600 pl-2 border-b-2 border-b-slate-600 ">{i+1}</td>
						 <td className="border-l-2 border-l-slate-600 pl-2 border-b-2 border-b-slate-600 text-center text-sm">{assign_name}</td>
						 <td className="border-l-2 border-l-slate-600 pl-2 border-b-2 border-b-slate-600 flex flex-col items-center text-center text-sm text-xs"><>{deadlineDate}</><br/><>{deadlineTime}</></td>
             <td className="border-l-2 border-l-slate-600 pl-2 border-b-2 border-b-slate-600 text-center text-sm text-xs">{links}</td>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
						...
						<button>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 border border-gray-700 rounded-full p-1 cursor-pointer">
  <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
						...
						:
						<button className='px-2 py-1 bg-accent rounded-md text-xs hover:scale-105 transition-all text-white cursor-pointer'>Submit</button>
						}
						 </td>
					</tr>
                  )
