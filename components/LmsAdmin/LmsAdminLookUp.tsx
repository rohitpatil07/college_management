  ...
  const get_faculty=async()=>{
    ...
    if(selected_serial_no==1){
      if(modal_current_input==''){
        ...
        const response = await axios.get(
        `http://localhost:5000/lms/filter/faculty/IT/${modal_current_input}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${AuthData.user.token}`,
        ...
        <div className="border-t-4 my-2 py-3 w-11/12 flex flex-row flex-wrap items-end justify-end ">
								...
								<div className="py-1">
                  {
										serial_text.map((value:string,x:number)=>(
											<button
											key={x}
                      onClick={()=>{on_serial_click(x);setShowFilters(!showFilters);}}
...
{modal ? <div className='w-screen h-screen fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50'>
          <div className='bg-white border-solid border-2 border-neutral-200 rounded-lg px-4 mx-auto sm:mx-0 w-11/12 sm:w-5/12'>
            <div className='border-b-2 border-gray-900 py-2'>
              <h3 className="text-lg sm:text-xl font-semibold text-gray-900">Search</h3>
            </div>
            ...
            onChange={(e)=>{setmodal_current_input(e.target.value)}}
						type="text"
					></input>
				</div>
            <div className='flex justify-end items-center w-100 border-t text-white p-3'>
              <button onClick={()=>{get_faculty();setmodal(false)}} className="px-3 py-1 rounded bg-green-600 text-white hover:bg-green-700 mr-2">
                  Search
                </button>
                <button onClick={()=>{setmodal(false)}} className="px-3 py-1 rounded bg-red-600 text-white hover:bg-red-700">
                  Close
                </button>
            </div>
          </div>
