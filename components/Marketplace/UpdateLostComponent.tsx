...
const UpdateLostComponent = ({ item_id }: any) => {
	const AuthData: any = useAuth();
    const [item,setItem] = useState(item_id);
	...
	const handleSubmit = async () => {
		...
		try {
			const response = await axios.post(
				"http://localhost:5000/market/lost_items/update",
				{ item_id, item_data },
				{
					headers: {
	...
	const getCurrentItem = async (item_id: any) => {
		try {
			const response = await axios.post(
	...
	useEffect(() => {
		setItem(parseInt(item_id))
		getCurrentItem(parseInt(item_id));
	}, []);
			...
			<div className="flex justify-center mt-5">
				<div className="bg-white w-11/12 sm:w-[75%] rounded-2xl p-5 flex flex-col justify-center items-center shadow-xl">
					<div className="mx-auto w-11/12 flex flex-wrap justify-start container py-3 text-slate-500 font-medium border-b">
						...
						<p className="mx-2">/</p>
						<Link
							href="/marketplace/lostfound/updateLostItem"
							className="flex flex-row items-center border-slate-300"
						>
							Update Lost Item
						</Link>
...
export default UpdateLostComponent;
