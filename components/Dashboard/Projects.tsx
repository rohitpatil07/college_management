...
=======
	...
	const save = async (r: number) => {
		setUpdateLoading(true);
		var project_id =
			`${AuthData.user.userData.user.roll_no}` + `${projectInfo.length + 1}`;
		let k: any = Projects[r];
		let project: any = {
			proj_id: project_id,
			roll_no: `${AuthData.user.userData.user.roll_no}`,
                  ...
                  <div className='flex flex-col items-center mx-auto mb-3 w-11/12 p-2 bg-white border-2 border-neutral-300 drop-shadow-xl rounded-lg' key={id}>
                    <div className='w-11/12'>
                      <div className='flex flex-row items-center justify-between w-full text-black mb-2 border-b-gray-300 border-b-4 pb-2'>
                        <h2 className='text-xl sm:text-2xl font-bold text-gray-900'>Project {id}</h2>
		...
		<div className="w-full sm:w-11/12 mx-auto  flex flex-col items-center justify-around bg-slate-200 sm:bg-white container rounded-lg">
			...
			{loading ? (
				...
				<>
					{projectInfo.map(
						(
							{ proj_id, proj_name, tech_stack, role, proj_desc, editing }: any,
							x: number
						) => (
							<div
							...
							>
								...
								<div className="flex flex-col-reverse sm:flex-wrap sm:flex-row items-center justify-between">
									{editing ? (
										<input
											type="text"
											onChange={(e) => {
												UpdateExistingData("proj_name", e.target.value, x);
											}}
											value={proj_name}
								...
								{editing ? (
									<input
										type="text"
										onChange={(e) => {
											UpdateExistingData("tech_stack", e.target.value, x);
										}}
										value={tech_stack}
								...
								{editing ? (
									<textarea
										...
										value={proj_desc}
										onChange={(e) => {
											UpdateExistingData("proj_desc", e.target.value, x);
										}}
									...
									>
										<div className="w-11/12">
											...
											<input
												...
												value={proj_name}
												onChange={(e) => {
													UpdateData("proj_name", e.target.value, x);
												}}
										...
										<div className="w-11/12">
											<input
												placeholder="Tech Stack Used eg. Python ,Node ,NextJS, etc."
												type="text"
												value={tech_stack}
												onChange={(e) => {
													UpdateData("tech_stack", e.target.value, x);
												}}
												className="mb-5 mx-auto w-full bg-white border-gray-300 border-solid border-2 rounded-lg text-black py-2 px-2"
											></input>
										</div>
										...
										<div className="w-11/12">
											<textarea
												...
												value={proj_desc}
												onChange={(e) => {
													UpdateData("proj_desc", e.target.value, x);
												}}
												className="mb-5 mx-auto w-full bg-white border-gray-300 border-solid border-2 rounded-lg text-black py-2 px-2"
												name="txtname"
												rows={4}
					...
					{Projects.length < 3 - projectInfo.length ? (
						<button
							className="p-2 w-11/12 sm:w-5/12 mx-auto px-5 rounded-md mb-7"
							style={{ backgroundColor: "#c9243f", color: "white" }}
							onClick={() => {
								addProjects(Projects.length);
							}}
						>
							Add More Projects
						</button>
