...
const WorkExperience = () => {
  ...
  const save = async (r: number) => {
    setUpdateLoading(true);
    var working_id = `${AuthData.user.userData.user.roll_no}` + `${workExps.length + 1}`;
  ...
  return (
    <div className="w-full sm:w-11/12 mx-auto  flex flex-col items-center justify-around bg-slate-200 sm:bg-white container rounded-lg">
      ...
      {loading ?
        ...
        :
        <>
          {workExps.map(({ work_id, company_name, location, role, description, start_month, end_month, editing }: any, x: number) =>
            <div className='flex flex-col drop-shadow-xl text-black mx-auto my-5 py-1 px-3 w-11/12 bg-white border-2 border-neutral-300 rounded-lg' key={work_id}>
              {
                editing ?
                  <div className='w-full flex flex-row mx-auto items-center justify-end my-2 border-b-gray-300 border-b-4 pb-2'>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" onClick={() => { changeEditing(work_id) }} className="w-4 h-4 sm:w-6 sm:h-6 ml-auto hover:scale-75 cursor-pointer">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                  </div>
                  :
                  <div className='w-full flex flex-row mx-auto items-center justify-end my-2 border-b-gray-300 border-b-4 pb-2'>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" onClick={() => { changeEditing(work_id) }} className="w-4 h-4 sm:w-6 sm:h-6 mr-2 hover:scale-75 cursor-pointer">
              ...
              <div className='flex flex-col-reverse sm:flex-wrap sm:flex-row items-center justify-between'>
                {
                  editing ? <input type='text' onChange={(e) => { UpdateExistingData('company_name', e.target.value, x) }} value={company_name} placeholder='Company Name' className='my-2 mr-2 bg-white border-gray-300 border-solid border-2 rounded-mg text-black p-1' />
                    : <h2 className='text-xl font-semibold text-center'>{company_name}</h2>
                }
                {
                  editing ?
                    <div className='flex flex-row flex-wrap'>
                      <input type='month' value={start_month} onChange={(e) => { UpdateExistingData('start_month', e.target.value, x) }} className='my-2 mr-2 bg-white border-gray-300 border-solid border-2 rounded- text-black p-2' />
                      <input type='month' value={end_month} onChange={(e) => { UpdateExistingData('end_month', e.target.value, x) }} className='my-2 bg-white border-gray-300 border-solid border-2 rounded-lg text-black p-2' />
                    </div>
                    : <h2 className='text-sm'>{start_month} - {end_month}</h2>
                }
              </div>
              {
                editing ? <input type='text' onChange={(e) => { UpdateExistingData('location', e.target.value, x) }} value={location} placeholder='Location' className='my-2 bg-white border-gray-300 border-solid border-2 rounded-lg text-black p-2' />
              ...
              {
                editing ? <textarea value={description} onChange={(e) => { UpdateExistingData('description', e.target.value, x) }} placeholder='Work Description' className='my-2 bg-white border-gray-300 border-solid border-2 rounded-lg text-black p-21' rows={4}></textarea>
          ...
          {workExps.length < 3 ? (
            <>
              {
                WorkExperience.map(({ id, company_name, location, role, description, start_month, end_month, disabling }: any, x: number) => (
                  <div className='flex flex-col items-center mx-auto mb-3 w-11/12 p-2 bg-white border-2 border-neutral-300 drop-shadow-xl rounded-lg' key={id}>
                    <div className='w-11/12'>
                      ...
                      <input placeholder='Company Name' type='text' value={company_name} onChange={(e) => { UpdateData('company_name', e.target.value, x) }} className='mb-5 mx-auto w-full bg-white border-gray-300 border-solid border-2 rounded-lg text-black py-2 px-2'></input>
                    </div>
                    <div className='w-11/12'>
                      <input placeholder='Location eg. Work From Home, Banglore, Mumbai, etc.' type='text' value={location} onChange={(e) => { UpdateData('location', e.target.value, x) }} className='mb-5 mx-auto w-full bg-white border-gray-300 border-solid border-2 rounded-lg text-black py-2 px-2'></input>
                    </div>
                    <div className='w-11/12'>
                    ...
                    <div className='w-11/12'>
                      <textarea placeholder='Description - share your experience and tech stacks learned' value={description} onChange={(e) => { UpdateData('description', e.target.value, x) }} className='mb-5 mx-auto w-full bg-white border-gray-300 border-solid border-2 rounded-lg text-black py-2 px-2' name="txtname" rows={4}></textarea>
                    </div>
                    <div className='w-11/12'>
                      <div className='flex mx-auto flex-col sm:flex-row w-full justify-around sm:items-center mb-2 border-b-2 pb-4'>
                        <h2 className='my-1 sm:my-0 text-sm sm:text-base text-slate-700 font-medium'>Start Month</h2>
                        <input type='month' value={start_month} onChange={(e) => { UpdateData('start_month', e.target.value, x) }} className='my-1 sm:my-0 mx-auto w-full md:w-4/12 bg-white border-gray-300 border-solid border-2 rounded-lg text-black py-2 px-2'></input>
                        <h2 className='my-1 sm:my-0 text-sm sm:text-base text-slate-700 font-medium'>End Month</h2>
                        <input type='month' value={end_month} onChange={(e) => { UpdateData('end_month', e.target.value, x) }} className='my-1 sm:my-0 mx-auto w-full  md:w-4/12 bg-white border-gray-300 border-solid border-2 rounded-lg text-black py-2 px-2'></input>
                      </div>
                    </div>
                    {disabling ? <button
                      disabled
          ...
          {WorkExperience.length < (3 - workExps.length) ? (<button className='p-2 w-11/12 sm:w-5/12 mx-auto px-5 rounded-md mb-7 hover:scale-105 transition-all' style={{ backgroundColor: '#c9243f', color: 'white' }} onClick={() => { addWorkExperience(WorkExperience.length) }}>Add Work Experience</button>) : ''}
        </>
      }
    </div>
  );
