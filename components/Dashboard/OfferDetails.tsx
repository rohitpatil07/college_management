...
const OfferDetails = () => {
  ...
  const getOfferLetter = async (offer_id: string) => {
    const response = await axios
      .get(`${server}/image/offerdownload/${offer_id}`, {
        headers: {
          "Content-Type": "application/json",
        },
        withCredentials: true,
    ...
    <div className="w-full sm:w-11/12 p-3 mx-auto flex flex-col items-center justify-around bg-slate-200 sm:bg-white container rounded-lg">
      ...
      {offers ? (
        <>
          {offers.map(
            (
              { company_name, offer_id, packages, offer_letter, showForm }: any,
              i: number
            ) => (
              <div
                key={offer_id}
                className="flex flex-col bg-white border-solid border-2 border-neutral-200 rounded-lg drop-shadow-xl w-full sm:w-11/12 mb-3 p-4 overflow-auto"
              >
                <h3 className="text-lg font-semibold text-gray-900 border-b-2 border-b-gray-400 pb-1 mb-1">
                  {i == 0
                    ? "First Offer"
                    : i == 1
                    ? "Second Offer"
                ...
                {offer_letter ? (
                  <div className="flex justify-end items-center w-100 text-white p-1 border-t-2 pt-2 mt-1">
                    ...
                    <button
                      className="px-3 py-1 rounded bg-accent text-white hover:bg-red-700"
                      onClick={() => {
                        update_Change(i);
                      }}
                    >
                  ...
                  <div
                    className="flex justify-end items-center w-100 text-white p-1 border-t-2 pt-2 mt-1"
                    onClick={() => {
                      update_Change(i);
                    }}
                  >
