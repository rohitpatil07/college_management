...
const AttendanceRecord = () => {
  ...
  const get_attendance_record = async () => {
    ...
    for (let i = 0; i < response.data.length; i++) {
      response.data[i]["abs_count"] = (
        response.data[i]["absent"].match(/IT/g) ||
        response.data[i]["absent"].match(/CS/g) ||
        response.data[i]["absent"].match(/EX/g) ||
        response.data[i]["absent"].match(/IN/g)
      ).length;
      response.data[i]["pres_count"] = (
        response.data[i]["present"].match(/IT/g) ||
        response.data[i]["present"].match(/CS/g) ||
        response.data[i]["present"].match(/EX/g) ||
        response.data[i]["present"].match(/IN/g)
      ).length;
    }
        ...
        {attend == null ? (
          ...
          <div className="w-full px-2">
            <table className="w-full table-auto border-separate border-spacing-2 border-slate-500 bg-red">
              <thead>
                <tr>
                  <th className="border-2  border-slate-600">Date</th>
                  <th className="border-2  border-slate-600">Stu. Present</th>
                ...
                {attend.map(
                  ({
                    ...
                    <tr key={attendance_id}>
                      ...
                      <td className="border-2 border-slate-600">{abs_count}</td>
                      <td className="border-2 border-slate-600">
                        <button
                          onClick={() => {
                            fetchAttendanceDetail(date.slice(0, 10));
                          }}
                          className="px-2 py-1 rounded-md cursor-pointer bg-blue-500 text-white hover:bg-blue-700 transition-all"
                        >
                          View
                        </button>
                      </td>
            ...
            <div className="w-full">
              <button
                className="ml-[50%] my-2 bg-accent text-white px-2 py-1 rounded-md hover:scale-105 transition-all w-fit"
                onClick={() => {
                  getExcel();
                }}
              >
          ...
          <div className="bg-white border-solid border-2 border-neutral-200 rounded-lg px-4 mx-auto sm:mx-0 w-11/12 sm:w-5/12">
            ...
            <table className="w-full table-auto border-separate border-spacing-2 border-slate-500 bg-red">
              ...
              {dataDisplay ? (
                <tbody>
                  {presents.map(
                    ({ roll_no, first_name, last_name, division }: any) => (
                      <tr key={roll_no}>
                        <td className="border-l-2 border-b-2 border-b-slate-600 border-l-slate-600">
                          {roll_no}
                        </td>
