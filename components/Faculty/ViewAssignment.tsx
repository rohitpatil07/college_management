  ...
  const get_material = async () => {
    ...
    for (let i = 0; i < response.data.student_submissions.length; i++) {
      ...
      if (mm > parseInt(deadline[0].slice(5, 7))) {
        ...
        if (diff == 1) {
          response.data.student_submissions[i]["due"] = "By " + diff + " month";
        } else {
        ...
        if (diff == 1) {
          ...
          response.data.student_submissions[i]["due"] = "By " + diff + " days";
        }
      } else if (hour > parseInt(deadline[1].slice(0, 2))) {
        let diff = hour - parseInt(deadline[1].slice(0, 2));
        if (diff == 1) {
          response.data.student_submissions[i]["due"] = "By " + diff + " hour";
        } else {
          response.data.student_submissions[i]["due"] = "By " + diff + " hours";
        }
            ...
            {readingmaterial.length == 0 ? (
              ...
              <div className="w-full px-2">
                <div className="flex flex-wrap items-center justify-between border-b-2 border-b-gray-500 my-1">
                  <input
                    onChange={(e) => getAssByRoll(e.target.value)}
                    className="rounded-md border border-gray-700 py-1 px-1 my-1"
                    placeholder="Enter Roll No."
                    type="text"
                  />
                  <button
                    onClick={downloadZip}
                    className="my-1  w-fit px-4 py-2 rounded-md bg-gray-500 text-white hover:scale-105 transition-all flex items-center justify-between text-sm"
                  >
                    Download All
                    <svg
                      stroke="white"
                    ...
                    >
                      <path
                        fill="#00b8df"
                        d="M47 112h10v6H47zM37 118h10v6H37zM47 100h10v6H47zM37 106h10v6H37zM47 88h10v6H47zM37 94h10v6H37zM47 76h10v6H47zM37 82h10v6H37zM47 64h10v6H47zM37 70h10v6H37zM56 29H40c-1.1 0-2 .9-2 2v25c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V31c0-1.1-.9-2-2-2zm-3 16H43V34h10v11zM104 80c-13.255 0-24 10.745-24 24s10.745 24 24 24 24-10.745 24-24-10.745-24-24-24zm10.882 16.988-.113.176-8.232 11.438c-.548.866-1.508 1.398-2.537 1.398s-1.989-.532-2.536-1.397l-8.346-11.614a3.01 3.01 0 0 1 .01-2.994 3.01 3.01 0 0 1 2.596-1.494H100V86c0-1.654 1.346-3 3-3h2c1.654 0 3 1.346 3 3v6.5h4.276c1.065 0 2.061.572 2.596 1.494a3.01 3.01 0 0 1 .01 2.994z"
                      />
                      <path
                        fill="#ff9a30"
