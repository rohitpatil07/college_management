...
const Module = ({module_id,subject_id,module_name,module_number}: any) => {
  ...
  const get_material = async () => {
	...
	console.log(assignment_id,file_name,file_type)
      const response = await axios.get(`http://localhost:5000/lms/download/getmaterial/${assignment_id}`, {
			headers: {
				'Content-Type': 'application/json',
				'Authorization': `Bearer ${AuthData.user.token}`
			},
