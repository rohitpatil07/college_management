...
const Assignment = ({ subject_id, subject_name }: any) => {
  ...
  const downloadAssignment = async (
    ...
    const response = await axios
      .get(`${server}/lms/download/getassignment/${assignment_id}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${AuthData.user.token}`,
        },
                  ...
                  (
                    ...
                    <tr key={i}>
                      ...
                      <td className="border-l-2 border-l-slate-600 pl-2 border-b-2 border-b-slate-600 text-center text-xs">
                        {edit ? (
                          <input
                            value={editAssign[i]["assign_name"]}
                            onChange={(e) =>
                              onChangeEdit(i, "assign_name", e.target.value)
                            }
                            className="h-6 text-xs text-black bg-white border-gray-500 border-2 rounded-md border-opacity-50 outline-none focus:border-blue-600 placeholder-gray-700 placeholder-opacity-0 transition-duration-200"
                            type="text"
                          />
                        ) : (
                          <>{assign_name}</>
                      ...
                      <td className="border-l-2 border-l-slate-600 pl-2 border-b-2 border-b-slate-600 text-center text-xs p-0">
                        {edit ? (
                          <input
                            value={editAssign[i]["assign_des"]}
                            onChange={(e) =>
                              onChangeEdit(i, "assign_des", e.target.value)
                            }
                            className="h-6 text-xs text-black bg-white border-gray-500 border-2 rounded-md border-opacity-50 outline-none focus:border-blue-600 placeholder-gray-700 placeholder-opacity-0 transition-duration-200"
                            type="text"
                          />
                      ...
                      <td className="border-l-2 border-l-slate-600 pl-2 border-b-2 border-b-slate-600 text-center text-xs">
                        {edit ? (
                          <input
                            onChange={(e) =>
                              onChangeEdit(i, "deadlineAt", e.target.value)
                            }
                            className="h-6 text-xs text-black bg-white border-gray-500 border-2 rounded-md border-opacity-50 outline-none focus:border-blue-600 placeholder-gray-700 placeholder-opacity-0 transition-duration-200"
                            type="datetime-local"
                          />
                        ) : (
                          <>{deadlineAt.slice(0, deadlineAt.indexOf("T"))}</>
                        )}
                      </td>
                      <td className="border-l-2 border-l-slate-600 pl-2 border-b-2 border-b-slate-600 text-center text-xs">
                        {edit ? (
                          <>
                            <input
                              onChange={(e) => {
                                onChangeEdit(i, "file", e);
                              }}
                              className="rounded-md border border-gray-300 md:w-11/12 my-1"
                              type="file"
                            />
                          </>
                        ) : (
                        ...
                        {edit ? (
                          <div className="flex flex-wrap items-center justify-around">
                            {eloading ? (
                              ...
                              <button
                                onClick={() => {
                                  editForm(i, assignment_id);
                                }}
                                className="p-1 my-1 text-sm bg-accent text-white font-semibold  rounded-md"
                              >
                                Save
                              </button>
                            )}
        ...
        <div className="w-screen h-screen fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50">
          <div className="bg-white border-solid border-2 border-neutral-200 rounded-lg px-4 mx-auto sm:mx-0 w-11/12 sm:w-5/12">
            ...
            <div className="my-4 w-full flex justify-center items-center">
              {assignload ? (
                <button className="p-2 w-fit mx-auto px-8 py-2 rounded-md bg-gray-500 text-white">
                  Adding
                </button>
              ) : (
                <button
