...
function Page() {
  ...
  const handleSubmit = async () => {
    ...
    if (response.status === 200 && "Notified" in response.data) {
      ...
      Swal.fire({
        icon: "error",
        title: "Notification was not sent..Please Try Again",
        showConfirmButton: false,
        timer: 1500,
      });
    }
  ...
  return (
    <div className="w-full flex flex-col justify-center items-center align-middle ">
      <h3 className="text-xl sm:text-2xl mt-8 font-bold text-gray-900">
        Send Notifications
      </h3>
      <div className="grid bg-white mt-5 w-[80%] grid-cols-2 px-20 pt-8 items-center rounded-2xl drop-shadow-lg gap-x-10 gap-y-4 py-[20px]">
        <div className="flex flex-row justify-between items-center text-sm sm:text-base text-slate-700 font-medium">
          ...
          <input
            ...
            className="bg-white border-slate-400 border-[1px] rounded-md"
          />
        </div>
        <div className="flex flex-row  justify-between items-center text-sm sm:text-base text-slate-700 font-medium">
          <p> Live KT</p>
      ...
      <div className=" w-[80%] my-[5px] flex flex-col bg-white px-20 pt-8 mt-6 items-center rounded-2xl drop-shadow-lg gap-x-10 gap-y-4 py-[20px]">
        <div className="flex w-full justify-start items-start">
          ...
          <textarea
            ...
            cols={80}
            rows={2}
            className="ml-[10px]  bg-white border-slate-400 border-[1px] rounded-md outline-none"
          ></textarea>
        </div>
        <div className="flex w-full justify-start items-start">
          <p>Message :</p>
      ...
      <button
        ...
        className="bg-[#C9243F] py-[5px] w-[100px] mt-[10px]  text-white hover:bg-green-500 rounded-md"
      >
        Submit
      </button>
    </div>
